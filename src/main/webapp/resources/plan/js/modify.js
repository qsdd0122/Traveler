$(document).ready(function() {
	...
	for(var i=0;i<allInfo.length;i++){
		var position = {title : allInfo[i].title, lonlat : new Tmapv2.LatLng(allInfo[i].mapX, allInfo[i].mapY), addr : allInfo[i].addr, img :  allInfo[i].img_src, contentId : allInfo[i].contentId, contentTypeId : allInfo[i].contentTypeId, overview : allInfo[i].overview };
	...
	$.ajax({
		...
		success : function(data){
			reco_markers=setListItems(data,reco_markers,map,param.pageNo,modify_position);
...
//		alert($('.sidebar-content').scrollTop());
		...
		if($('.sidebar-content').prop('scrollHeight')==Math.ceil($('.sidebar-content').scrollTop()+$('.sidebar-content').height())) { 
			if(flag==0){
				...
				reco_markers=callAjaxRecommend(param,reco_markers,map,modify_position); //ajax 호출
				ex_markers=reco_markers;
			}
			if(flag == 2){
				keywordParam.pageNo++;
				console.log("keyword" + keyword_markers);
				keyword_markers=callAjaxKeyword(keywordParam,keyword_markers,map,modify_position);
				ex_markers=keyword_markers;
			}
			if(flag == 3){
				keywordParam.pageNo++;
				console.log("keyword" + keyword_markers);
				keyword_markers=callAjaxKeywordTmap(keywordParam,keyword_markers,map,modify_position);
				ex_markers=keyword_markers;
			}
		} 
	}); 
	...
	$("#btn-search").click(function(){
		...
		$.ajax({   
			...
			success :  function(data){
				if(data.length == 0 ){
					...
					$.ajax({   
						...
						success :  function(data){
							...
							if(data.length != 0){
								keyword_markers=TmapItems(data,keyword_markers,map,keywordParam.pageNo,modify_position);
								ex_markers = keyword_markers;
							}
						},
						error : function(error) {
				...
				if(data.length != 0){
					keyword_markers=keywordItems(data,keyword_markers,map,keywordParam.pageNo,modify_position);  //테스트 데이터 리스트 입니다.
					ex_markers = keyword_markers;
				}
			},
			error : function(error) {
	...
	$("#bookmark").click(function(){
		...
		$("#myplan").attr("disabled",false);
		if(ex_markers != false) deleteMarkers(ex_markers);
		bmk_positions = loadBookmark(modify_position);
		$("#ul-recommend").hide();
		$("#ul-search").hide();
		$("#ul-bookmark").show();
		$("#ul-myPlan").hide();
	...
	$.ajax({   
		...
		success :  function(data){
			...
			if(data.length != 0){
				...
				keyword_markers=TmapItems(data,keyword_markers,map,keywordParam.pageNo,modify_position);  //테스트 데이터 리스트 입니다.
...
function callAjaxKeyword(keywordParam,keyword_markers,map,modify_position) {
	$.ajax({   
		url : "/spot/keyword",
		type : "get",
		dataType : "json",
...
function callAjaxRecommend(param,reco_markers,map,modify_position,modify_position) {
	$.ajax({   
		url : "/spot/information",
		type : "get",
		dataType : "json",
		...
		success : function(data){
			...
			if(data.length != 0){
				//testLoading.show(); //로딩 on(로딩바가 있을경우만 넣습니다. 없을경우 빼셔도 상관 없습니다.)
				reco_markers=setListItems(data,reco_markers,map,param.pageNo,modify_position);  //테스트 데이터 리스트 입니다.
				//testLoading.hide(); //로딩 off(로딩바가 있을경우만 넣습니다. 없을경우 빼셔도 상관 없습니다.)
			}   
		},
		error : function(error) {
...
function setListItems(data,markers,map,pageNo,modify_position){
	var positions=[];
	...
	$('.spot_info'+pageNo).each(function(i){
		...
		$(this).click(function(){
			...
			marker.setVisible(true);
			for(var i=0;i<modify_position.length;i++) {
				if(modify_position[i].isDelete != 'Y') {
					if(JSON.stringify(lonlat) === JSON.stringify(modify_position[i].lonlat)) {
						if(title == modify_position[i].title) {
...
//			alert("기본markerClick")
		...
		$("#addModal").modal("hide");
		var strArr = ($("#plandate").val()).split("-");
		var stDate = new Date(parseInt(strArr[0]), parseInt(strArr[1])-1, parseInt(strArr[2]));
...
function modifyAddPlan(position,map,marker,planDay,modify_position){
	...
	var content = $('<li class="list_add_content day'+planDay+'-'+liIndex+'"><hr><div class="row testRemove">'
			+'<div class="col-lg-5" style="background-color : #f5f5f5"><img class="img-responsive" class="plan_photo"'
			+'style="cursor: pointer;" src="'+position.img+'" alt="" width="150" height="100">'
			+'</div><div class="col-lg-7" style="background-color : #f5f5f5">'
			+'<h5 class="plan_title">'+position.title+'</h5>'
...
//		delete modify_position['lonlat'];
	...
	var strArr = ($("#plandate").val()).split("-");
	var stDate = new Date(parseInt(strArr[0]), parseInt(strArr[1])-1, parseInt(strArr[2]));
	stDate.setDate(stDate.getDate() + (planDay -1));
...
//	position.planTitle = $("#plantitle").val();
	...
	modifyMarkerClick(map,marker,position.lonlat,position.title,position,modify_position);
//	delete position['overview'];
	var position_data = JSON.stringify(position);
	position.positionData = position_data;
...
function modifyMarkerClick(map,marker,latlng,title,position,modify_position){
	if(position.overview == null) position.overview = "";
	marker.addListener("click",function(evt){
		var content= "<div style='position: static; display: flex; flex-direction: column; font-size: 14px; box-shadow: 5px 5px 5px #00000040; border-radius: 10px; top: 410px; left : 800px; width : 250px; background: #FFFFFF 0% 0% no-repeat padding-box;'>"+
		"<div class='img-box' style='position: relative; width: 100%; height: 150px; border-radius: 10px 10px 0 0 ;no-repeat center;'>"+
...
function loadBookmark(modify_position){
	var positions=[];
	$.ajax({
		url : "/plan/bookmark",
		type : "get",
		...
		success : function(data) {
			$.each(data, function(key, val){
				...
				for(var i=0;i<modify_position.length;i++) {
					if(modify_position[i].isDelete != 'Y') {
						if(JSON.stringify(new Tmapv2.LatLng(data[key].mapX, data[key].mapY)) === JSON.stringify(modify_position[i].lonlat)) {
							if(data[key].title == modify_position[i].title) {
								isDiff = false;
								break;
							}
						}
...
function keywordItems(data,markers,map,pageNo,modify_position){
	var positions=[];
	$.each(data, function(i, result) {
...
function TmapItems(data,markers,map,pageNo,modify_position){
	var positions=[];
	$.each(data, function(i, result) {
